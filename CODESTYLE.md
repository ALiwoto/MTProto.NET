> #### NOTE: This code style standard for MTProto is a work in progress and much of the code does not currently conform to these rules.  This is something that will be addressed by the core team.
...
* Directory names should follow the namespace for the class after `MTProto`. For example, one would expect to find the public class `MTProto.Constants.WotoTools` in **MTProto\Constants\WotoTools.cs**.
